#!/usr/bin/env node
/**
 * PayRail Skill — Agent Invocation Harness
 * ----------------------------------------
 * A runnable demonstration that an AI agent can invoke the PayRail skill exactly the way
 ...
 *   3. read references/payrail.md            (the exact cast/forge command for that capability)
 ...
 *
 * Usage:
 *   PRIVATE_KEY=0x... node examples/agent/payrail-agent.mjs "<natural language request>"
 *
 * Examples:
 ...
 *   node examples/agent/payrail-agent.mjs "pay 0.004 PHRS to 0x00000000000000000000000000000000C0ffee00 for invoice-777"
 *   node examples/agent/payrail-agent.mjs "verify a payment of 0.004 PHRS to 0x...C0ffee00 for invoice-777"
...
const RAIL = process.env.RAIL || "0xdfDf119964C7858905FbE7175Ff32fdD509dEc50";
...
function loadCapabilityIndex() {
  ...
  const rows = md
    .split("\n")
    .filter((l) => l.trim().startsWith("|") && l.includes("references/payrail.md"))
...
// ---- capabilities (each maps 1:1 to a references/payrail.md section) ----------------
function doBalance(intent) {
  ...
  step(3, "reference: references/payrail.md#reads → balanceOf(address,address)");
  const wei = cast(["call", RAIL, "balanceOf(address,address)(uint256)", who, ZERO, "--rpc-url", RPC]);
  const phrs = cast(["from-wei", wei.split(" ")[0]]);
  step(7, `parsed: PayRail balance of ${who} = ${phrs} PHRS`);
}

function doDeposit(intent) {
  const amount = num(intent);
  if (!amount) throw new Error("could not parse an amount to deposit");
  step(3, "reference: references/payrail.md#deposit-funds → depositNative()");
  preChecks(`deposit ${amount} PHRS`);
  const wei = toWei(amount);
  const out = cast(["send", RAIL, "depositNative()", "--value", wei, "--rpc-url", RPC, "--private-key", KEY, "--legacy"], { send: true });
...
function buildAuth(intent, { forVerify = false } = {}) {
  ...
  step(forVerify ? 3 : 3, "reference: references/payrail.md#build--sign-a-voucher → hashAuthorization");
  const digest = cast(["call", RAIL, `hashAuthorization(${AUTH_TUPLE})(bytes32)`, tuple, "--rpc-url", RPC]);
...
function doVerify(intent) {
  ...
  step(5, "reference: references/payrail.md#verify → verify(auth,sig)");
  const out = cast(["call", RAIL, `verify(${AUTH_TUPLE},bytes)(bool,string)`, tuple, sig, "--rpc-url", RPC]);
  step(7, `parsed: verify → ${out.replace(/\n/g, " ")}`);
}

function doPay(intent) {
  step(3, "reference: references/payrail.md#redeem (full voucher flow)");
  ...
  const v = cast(["call", RAIL, `verify(${AUTH_TUPLE},bytes)(bool,string)`, tuple, sig, "--rpc-url", RPC]);
  ...
  const out = cast(["send", RAIL, `redeem(${AUTH_TUPLE},bytes)`, tuple, sig, "--rpc-url", RPC, "--private-key", KEY, "--legacy"], { send: true });
  const used = cast(["call", RAIL, "nonceUsed(address,bytes32)(bool)", me(), nonce, "--rpc-url", RPC]);
...
function doWithdraw(intent) {
  ...
  step(3, "reference: references/payrail.md#withdraw-unspent-balance → withdraw(address,uint256)");
  preChecks(`withdraw ${amount} PHRS`);
  const out = cast(["send", RAIL, "withdraw(address,uint256)", ZERO, toWei(amount), "--rpc-url", RPC, "--private-key", KEY, "--legacy"], { send: true });
...
function main() {
  ...
  if (!intent) {
    console.error('Usage: node examples/agent/payrail-agent.mjs "<request>"');
    process.exit(1);
  }
